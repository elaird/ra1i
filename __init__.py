...
def scaled(t, factor):
    return tuple([factor*a if a is not None else None for a in t])


def excl(counts, isExclusive):
    out = []
    for i, count, isExcl in zip(range(len(counts)), counts, isExclusive):
        out.append(count if (isExcl or count is None) else (count - counts[i+1]))
    return tuple(out)


def itMult(l1=[], l2=[]):
    return tuple([a*b if a is not None else None for a, b in zip(l1, l2)])
...
vars = ["htBinLowerEdges", "htMaxForPlot",
        "lumi", "htMeans", "systBins",
        "observations", "triggerEfficiencies",
        "mcStatError", "fixedParameters"]
